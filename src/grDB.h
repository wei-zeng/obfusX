...
public:
	...
	std::unordered_set<int> pinNodeIDs;
...
struct ObfusOption {
	...
	Gnet newGnet;
	//std::map<size_t, int> changeInEdgeDemand, changeInViaDemand; // EdgeID -> change in demand
...
public:
	...
	void writeGlobalWires(const Layout &layout, const char *wiresFile) const;
	void writeGlobalWires(const Layout &layout, const string &wiresFile) const;
...
public:
	...
	vector<Vpin> getVpins(const Layout &layout, const int gnetID,
			const int splitLayer, const bool twoCutNetsOnly,
			bool includeAllPIOs) const;
	void outputCSV(const Layout &layout, const int splitLayer,
...
public:
	...
	// void layerAssignment(const Layout & layout, Graph & graph3D, const Point3D & root_pt);
	std::vector<EdgeType> kruskalMST(Graph &graphMST) const;

	std::pair<ObfusOption, bool> traverseObfusOpts(const Layout &layout,
			int layer, vector<Vpin> &vpins, size_t best_vpin,
...
bool isOptionValid(const ObfusOption &opt, const RoutingDB &routingDB);
