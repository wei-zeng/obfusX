...
public:
	...
	Polygon(lef::Polygon poly) {
		for (auto &pt : poly.points_)
			_vertices.emplace_back(static_cast<int>(round(pt.x_ * 2000)),
					static_cast<int>(round(pt.y_ * 2000)));
	}
	Polygon(int lx, int ly, int ux, int uy) {
		_vertices.emplace_back(lx, ly);
		_vertices.emplace_back(ux, ly);
		_vertices.emplace_back(ux, uy);
		_vertices.emplace_back(lx, uy);
	}
	...
	void rotate_reflect(int orient) {
		//for (auto &v : _vertices) {
		// lower priority TODO: not used
		//}
	}
	bool coversPoint(int x, int y) const { // (x,y) is inside or on the edge of polygon
		// check if on any edge
		size_t start = 0;
		for (size_t i = start; i < _vertices.size() + start; i++) {
			size_t s = i % _vertices.size(), t = (i + 1) % _vertices.size();
			if (_vertices[s]._x == _vertices[t]._x) {
				if (x == _vertices[s]._x
						&& y >= min(_vertices[s]._y, _vertices[t]._y)
						&& y <= max(_vertices[s]._y, _vertices[t]._y)) {
					return true;
				}
			} else if (_vertices[s]._y == _vertices[t]._y) {
				if (y == _vertices[s]._y
						&& x >= min(_vertices[s]._x, _vertices[t]._x)
						&& x <= max(_vertices[s]._x, _vertices[t]._x)) {
					return true;
				}
			}
		}
		int cnt = 0;
		int tmp = 0; // 1: y == min(s.y, t.y), 2: y == max(s.y, t.y), 0: otherwise
		start = 0;
		while (_vertices[start]._y == y) {
			start++;
		}
		for (size_t i = start; i < _vertices.size() + start; i++) {
			size_t s = i % _vertices.size(), t = (i + 1) % _vertices.size();
			if (_vertices[s]._x == _vertices[t]._x && _vertices[s]._x > x) {
				assert(_vertices[s]._y != _vertices[t]._y);
				if (y > min(_vertices[s]._y, _vertices[t]._y)
						&& y < max(_vertices[s]._y, _vertices[t]._y)) {
					cnt++;
					tmp = 0;
				} else if (y == min(_vertices[s]._y, _vertices[t]._y)) {
					if (tmp == 0) {
						tmp = 1;
					} else {
						if (tmp == 2) {
				...
				} else if (y == max(_vertices[s]._y, _vertices[t]._y)) {
					if (tmp == 0) {
						tmp = 2;
					} else {
						if (tmp == 1) {
	...
	bool coversPoint(int x, int y, int extValue) const { // up to +/- extValue on x and y
		return coversPoint(x - extValue, y - extValue)
				|| coversPoint(x + extValue, y - extValue)
				|| coversPoint(x + extValue, y + extValue)
				|| coversPoint(x - extValue, y + extValue);
	}
};
