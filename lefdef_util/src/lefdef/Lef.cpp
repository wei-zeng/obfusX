...
int LefParser::set_via(lefrCallbackType_e, lefiVia *via, lefiUserData ud) {
	...
	for (int i = 0; i < via->numLayers(); i++) {
		...
		for (int j = 0; j < via->numRects(i); j++) {
			...
			the_via_layer.rect_vec_.emplace_back(via->xl(i, j), via->yl(i, j),
	...
	for (int i = 0; i < pin->numPorts(); i++) {
		...
		for (int j = 0; j < port->numItems(); j++) {
			if (port->itemType(j) == lefiGeomLayerE) {
				...
				cur_port->rects_.emplace_back(rect->xl, rect->yl, rect->xh,
						rect->yh);
			} else if (port->itemType(j) == lefiGeomPolygonE) {
				auto rect = port->getPolygon(j);
				cur_port->polygons_.emplace_back(rect->numPoints, rect->x,
						rect->y);
			}
		}
		...
		for (auto &r : cur_port->polygons_) {
			for (auto &pt : r.points_) {
				if (bbox.lx_ > pt.x_) {
					bbox.lx_ = pt.x_;
				}
				if (bbox.ly_ > pt.y_) {
...
ostream& operator<<(ostream &os, const Unit &u) {
	os << "Unit (db_name=" << u.db_name_ << ", db_number=" << u.db_number_
			<< ")";
	return os;
}

ostream& operator<<(ostream &os, const Site &s) {
	os << "Site (name=" << s.name_ << ", class=" << s.class_ << ", x=" << s.x_
			<< ", y=" << s.y_ << ")";
	return os;
}

ostream& operator<<(ostream &os, const Layer &l) {
	os << "Layer (name=" << l.name_ << ", type=" << l.type_ << ", dir="
			<< to_string(static_cast<long long>(l.dir_)) << ", min_width_="
			<< l.min_width_ << ", area=" << l.area_ << ", width=" << l.width_
			<< ", pitch=" << l.pitch_ << ", pitch_x=" << l.pitch_x_
			<< ", pitch_y=" << l.pitch_y_ << ")";
...
ostream& operator<<(ostream &os, const Macro &m) {
	os << "Macro (name=" << m.name_ << ", site_name=" << m.site_name_
...
ostream& operator<<(ostream &os, const Pin &p) {
	os << "Pin (name=" << p.name_ << ", dir="
			<< to_string(static_cast<long long>(p.dir_)) << ", use="
			<< to_string(static_cast<long long>(p.use_)) << ", num_ports="
			<< p.ports_.size() << ", bbox=" << p.bbox_ << ")";

	return os;
}

ostream& operator<<(ostream &os, const Port &p) {
	os << "Port (name=" << p.name_ << "layer=" << p.layer_name_ << "num_rects="
			<< p.rects_.size() << ")";
...
ostream& operator<<(ostream &os, const Rect &r) {
	os << "Rect (lx=" << r.lx_ << ", ly=" << r.ly_ << ", ux=" << r.ux_
			<< ", uy=" << r.uy_ << ")";
	return os;
}
