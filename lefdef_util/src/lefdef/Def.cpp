...
void Def::read_def(string filename) {
	...
	auto fp = unique_ptr<FILE, decltype(&fclose)>(fopen(filename.c_str(), "r"),
			&fclose);
...
int DefParser::set_track(defrCallbackType_e, defiTrack *track,
defiUserData ud) {
	auto def = static_cast<Def*>(ud);
...
int DefParser::set_gcell_grid(defrCallbackType_e, defiGcellGrid *gcell_grid,
defiUserData ud) {
	auto def = static_cast<Def*>(ud);
...
static void process_routed_net(NetPtr the_net, defiNet *net) {
	...
	for (int i = 0; i < net->numWires(); i++) {
		...
		the_net->vias_.reserve(the_net->vias_.size() + wire->numPaths());
		...
		for (int j = 0; j < wire->numPaths(); j++) {
			...
			while (path_id != DEFIPATH_DONE) {
				...
				case DEFIPATH_FLUSHPOINT:
					...
					break;
				case DEFIPATH_VIA:
					the_net->vias_.emplace_back(
							make_shared<Via>(path->getVia(),
									wire_segment->rpoints_.back()->x_,
									wire_segment->rpoints_.back()->y_));
					break;
				}
				path_id = path->next();
			}
...
static NetPtr create_net(Def *def, int dbu, defiNet *net) {
	...
	for (int i = 0; i < net->numConnections(); i++) {
		...
		if (comp == nullptr) {
			...
			c = make_shared<Connection>(pin_name, comp, lef_pin, lx, ly, ux,
					uy);
		}
...
int DefParser::set_special_net(defrCallbackType_e, defiNet *net,
defiUserData ud) {
	auto def = static_cast<Def*>(ud);
	auto dbu = def->pimpl_->dbu_;
...
ostream& operator<<(ostream &os, const Row &r) {
	os << "Row (name=" << r.name_ << ", macro=" << r.macro_ << ", orient_str="
			<< r.orient_str_ << ", orient=" << r.orient_ << ", x=" << r.x_
			<< ", y=" << r.y_ << ", num_x=" << r.num_x_ << ", num_y="
			<< r.num_y_ << ", step_x=" << r.step_x_ << ", step_y=" << r.step_y_
			<< ")";
...
ostream& operator<<(ostream &os, const Track &t) {
	os << "Track (direction=" << static_cast<int>(t.direction_) << ", location="
			<< t.location_ << ", num_tracks=" << t.num_tracks_ << ", step="
			<< t.step_ << ", num_layers=" << t.num_layers_ << ", layer="
			<< t.layer_ << ")";
...
ostream& operator<<(ostream &os, const Component &c) {
	os << "Component (name=" << c.name_ << ", ref_name=" << c.ref_name_
			<< ", x=" << c.x_ << ", y=" << c.y_ << ", is_fixed=" << c.is_fixed_
			<< ", is_placed=" << c.is_placed_ << ", orient_str="
			<< c.orient_str_ << ", orient=" << c.orient_ << ")";

	return os;
}

ostream& operator<<(ostream &os, const Pin &p) {
	os << "Pin (name=" << p.name_ << ", net_name=" << p.net_name_ << ", layer="
			<< p.layer_ << ", dir=" << static_cast<long long>(p.dir_) << ", x="
			<< p.x_ << ", y=" << p.y_ << ", orient_str=" << p.orient_str_
			<< ", orient=" << p.orient_ << ", lx=" << p.lx_ << ", ly=" << p.ly_
			<< ", ux=" << p.ux_ << ", uy=" << p.uy_ << ")";
...
ostream& operator<<(ostream &os, const Wire &w) {
	os << "Wire (wire_type=" << w.wire_type_ << ", num_wire_segments="
			<< w.wire_segments_.size() << ")";

	return os;
}

ostream& operator<<(ostream &os, const WireSegment &ws) {
	os << "WireSegment (layer_name=" << ws.layer_name_ << ", width="
			<< ws.width_ << ", num_rpoints=" << ws.rpoints_.size() << ")";

	return os;
}

ostream& operator<<(ostream &os, const Connection &c) {
	os << "Connection (name=" << c.name_ << ", lx=" << c.lx_ << ", ly=" << c.ly_
			<< ", ux=" << c.ux_ << ", uy=" << c.uy_;
...
ostream& operator<<(ostream &os, const Net &n) {
	os << "Net (name=" << n.name_ << ", num_pins=" << n.connections_.size()
			<< ", num_wires=" << n.wires_.size() << ", num_vias="
			<< n.vias_.size() << ")";
