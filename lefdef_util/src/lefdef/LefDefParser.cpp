 ...
 * Default ctor.
 */
MyLefDefParser::MyLefDefParser() :
		lef_(lef::Lef::get_instance()), def_(def::Def::get_instance()) {
	//
}
 ...
 */
MyLefDefParser& MyLefDefParser::get_instance() {
	static MyLefDefParser ldp;
	return ldp;
}
 ...
 */
void MyLefDefParser::read_lef(string filename) {
 ...
 */
void MyLefDefParser::read_def(string filename) {
 ...
 *
 */
void MyLefDefParser::write_bookshelf(string filename) const {
	cout << "Writing bookshelf aux file." << endl;
	ofstream ofs(filename + ".aux");
	ofs << "RowBasedPlacement : " << " " << filename << ".nodes" << " "
			<< filename << ".nets" << " " << filename << ".wts" << " "
			<< filename << ".pl" << " " << filename << ".scl" << " " << filename
			<< ".shapes" << " " << filename << ".route" << endl;
	ofs.close();
	...
	//route
	cout << "Writing bookshelf route file." << endl;
	write_bookshelf_route(filename + ".route");

	//gr
	cout << "Writing bookshelf gr file." << endl;
	write_bookshelf_gr(filename + ".gr");
}
 ...
 *
 */
void MyLefDefParser::write_bookshelf_nodes(string filename) const {
 ...
 *
 */
void MyLefDefParser::write_bookshelf_nets(string filename) const {
	ofstream ofs(filename);
 ...
 *
 */
void MyLefDefParser::write_bookshelf_wts(string filename) const {
	ofstream ofs(filename);
 ...
 *
 */
void MyLefDefParser::write_bookshelf_scl(string filename) const {
	ofstream ofs(filename);
 ...
 *
 */
void MyLefDefParser::write_bookshelf_pl(string filename) const {
	ofstream ofs(filename);
	...
	for (auto it : component_umap) {
		...
		if (c->is_placed_ || c->is_fixed_) {
			ofs << "\t" << c->x_ / x_pitch_dbu << "\t" << c->y_ / y_pitch_dbu
					<< "\t: " << c->orient_str_ << endl;
		} else {
			ofs << "\t" << 0 << "\t" << 0 << "\t: N" << endl;
		}
	...
	for (auto &it : pin_umap) {
		...
		ofs << "\t" << p->x_ / x_pitch_dbu << "\t" << p->y_ / y_pitch_dbu
				<< "\t: " << p->orient_str_ << endl;
	}
...
void MyLefDefParser::write_bookshelf_route(string filename) const {
	ofstream ofs(filename);
	ofs << "route 1.0" << endl;
	ofs << "# Created : ";
	ofs << get_current_time_stamp() << endl << endl;
...
void MyLefDefParser::write_bookshelf_gr(string filename) const {

}
/**
 *
 */
void MyLefDefParser::update_def(string bookshelf_pl) {
...
def::Def& MyLefDefParser::get_def() {
	return def_;
}
